    ...
    public class MonitoringDesignerService : IMonitoringDesigner
    {
            ...
            CancellationToken cancellationToken)
        {
            var missingComponents = components
                .Where(c => !existingMonitoring.Any(m => m.ComponentId == c.Id))
                .ToList();

            if (!missingComponents.Any()) return existingMonitoring;
            ...
            var additionalMonitoring = await _llamaService.GetStructuredResponseAsync<List<MonitoringDesign.ComponentMonitoring>>(
